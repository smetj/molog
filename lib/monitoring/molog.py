...
class connector():
	...
	def ignore_record(self,type=None,host=None,data=None):
		...
		for regex in regexes:
			if re.search(regex[1],data):
				self.logging.put(['Debug','Ignoring log for host %s with Regex ID %s (%s) -> %s'%(host,regex[0],regex[0],data)])
				return True
			else:
				self.logging.put(['Debug','No match for host %s with Regex ID %s (%s) -> %s'%(host,regex[0],regex[1],data)])
		return False
	...
	def get_messages(self,host):
		results={}
		querydb=self.get_db()
		query="select results.id,results.type,ReceivedAt,SysLogTag,Message from Syslog.SystemEvents join molog.results on molog.results.rsyslog_id = Syslog.SystemEvents.ID where molog.results.deleted=0 and Syslog.SystemEvents.FromHost='%s' order by ReceivedAt;"%(host)
		querydb.execute(query)
